...
class VentaController extends Controller
{
    ...
    public function show($id)
    {
    	$venta=DB::table('venta as v')
            ->join('persona as p','v.idcliente','=','p.idpersona')
            ->join('detalle_venta as dv','v.idventa','=','dv.idventa')
            ->select('v.idventa','v.fecha_hora','p.nombre','v.comentario','v.tipo_comprobante','v.serie_comprobante','v.num_comprobante','v.impuesto','v.estado','v.total_venta')
            ->where('v.idventa','=',$id)
            ->first();
    ...
    public function destroy($id)
    {
        ...
        if ($venta->estado=='C')
            {
                Session::flash('message','Proceso Cancelado..!!! La venta ya se encuentra ANULADA');
                return Redirect::to('ventas/venta');
            }
            else
            {
    ...
    public function reportec($id){
        ...
        $pdf::SetXY(187,153);
        $pdf::MultiCell(20,0,"$. ".sprintf("%0.2F", $venta->total_venta-($venta->total_venta*$venta->impuesto/100)));
        $pdf::SetXY(187,160);
        $pdf::MultiCell(20,0,"$. ".sprintf("%0.2F", ($venta->total_venta*$venta->impuesto/100)));
        $pdf::SetXY(187,167);
    ...
    public function reporte(){
         ...
         $pdf::Cell(0,10,utf8_decode("Listado Ventas: General"),0,"","C");
         $pdf::Ln();
         $pdf::Ln();
         ...
         foreach ($registros as $reg)
         {
            ...
            $pdf::cell(20,8,utf8_decode($reg->total_venta),1,"","R",true);
            $pdf::Ln(); 
         }
    ...
    public function reporteact(){
         ...
         $pdf::SetTextColor(35,56,113);
         $pdf::SetFont('Arial','B',11);
         $pdf::Cell(0,10,utf8_decode("Listado Ventas: Activas"),0,"","C");
         $pdf::Ln();
         $pdf::Ln();
         $pdf::SetTextColor(0,0,0);  // Establece el color del texto 
         $pdf::SetFillColor(206, 246, 245); // establece el color del fondo de la celda 
    ...
    public function reportecan(){
         ...
         $pdf::SetTextColor(35,56,113);
         $pdf::SetFont('Arial','B',11);
         $pdf::Cell(0,10,utf8_decode("Listado Ventas: Canceladas"),0,"","C");
         $pdf::Ln();
         $pdf::Ln();
         $pdf::SetTextColor(0,0,0);  // Establece el color del texto 
         $pdf::SetFillColor(206, 246, 245); // establece el color del fondo de la celda 
