...
class IngresoController extends Controller
{
     ...
     public function store (IngresoFormRequest $request)
    {
    	try{
	        ...
	        $ingreso->estado='A';
            $ingreso->comentario=$request->get('comentario');
	        $ingreso->save();
    ...
    public function show($id)
    {
    	$ingreso=DB::table('ingreso as i')
            ->join('persona as p','i.idproveedor','=','p.idpersona')
            ->join('detalle_ingreso as di','i.idingreso','=','di.idingreso')
            ->select('i.idingreso','i.fecha_hora','p.nombre','i.comentario','i.tipo_comprobante','i.serie_comprobante','i.num_comprobante','i.impuesto','i.estado',DB::raw('sum(di.cantidad*precio_compra) as total'))
    ...
    public function destroy($id)
    {
        ...
        if ($ingreso->estado=='C')
            {
                Session::flash('message','Proceso Cancelado..!!! La compra ya se encuentra ANULADA');
                return Redirect::to('compras/ingreso');
            }
            else
            {
    ...
    $ingreso=DB::table('ingreso as i')
        ...
        $pdf::SetXY(187,153);
        $pdf::MultiCell(20,0,"$. ".sprintf("%0.2F", $ingreso->total-($ingreso->total*$ingreso->impuesto/100)));
        $pdf::SetXY(187,160);
        $pdf::MultiCell(20,0,"$. ".sprintf("%0.2F", ($ingreso->total*$ingreso->impuesto/100)));
        $pdf::SetXY(187,167);
    ...
    public function reporte(){
         ...
         $pdf::Cell(0,10,utf8_decode("Listado Compras: General"),0,"","C");
         $pdf::Ln();
         $pdf::Ln();
    ...
    public function reporteact(){
         ...
         $pdf::SetTextColor(35,56,113);
         $pdf::SetFont('Arial','B',11);
         $pdf::Cell(0,10,utf8_decode("Listado Compras: Activas"),0,"","C");
         $pdf::Ln();
         $pdf::Ln();
         $pdf::SetTextColor(0,0,0);  // Establece el color del texto 
         $pdf::SetFillColor(206, 246, 245); // establece el color del fondo de la celda 
    ...
    public function reportecan(){
         ...
         $pdf::SetTextColor(35,56,113);
         $pdf::SetFont('Arial','B',11);
         $pdf::Cell(0,10,utf8_decode("Listado Compras: Canceladas"),0,"","C");
         $pdf::Ln();
         $pdf::Ln();
         $pdf::SetTextColor(0,0,0);  // Establece el color del texto 
         $pdf::SetFillColor(206, 246, 245); // establece el color del fondo de la celda 
